...
If not, unnecessary cuppling is created between the caller and the callee.
That unnecessary cuppling will have negative consequences in various aspects of the code like testeability, maintenability, and reusability.

Let's analyze an example of a low quality function definition:
...
Let's take a look on how that function signature hinders testeability, reusability and redeability of the function.
...
In the same line, every caller of the function must provide a full `CfdpStubConfig` object, this reduces de likely of being able to reuse this method.
As you might already experienced when reading the above code it is not very easy to followg what the code does.
To better highlight all these points let`s write a better definition of the function.

[,java]
----
public StubbedRemoteCfdpDownloadEntity(String entityId, String localFilestorePath, String remoteFilestorePath, FinishedTransactionsService finishedTrnsactionsService)
{
    super(entityId, localFilestorePath, remoteFilestorePath, finishedTrnsactionsService);
}
----

We can agree, that this new version of the function is easier to test, understand and reuse.
